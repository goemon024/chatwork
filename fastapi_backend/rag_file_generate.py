...
def add_to_index(doc_vector, index, doc_store):
    ...
    ### doc_vectorをjsonでアップロード ###
    filename = f"docs/{doc_vector['metadata']['id'][0]}.json"
    json_data = json.dumps(doc_vector, ensure_ascii=False)
    supabase.storage.from_("rag-files").upload(filename, json_data.encode("utf-8"))
    print(f"✅ 文書 {doc_vector['metadata']['id'][0]} をJSONでアップロード完了")
